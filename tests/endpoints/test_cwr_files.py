...
from tests.util.service import TestFileService
...
class TestSend(unittest.TestCase):
    def setUp(self):
        ...
        self._app.config['FILE_SERVICE'] = TestFileService()

    def test_get(self):
        client = self._app.test_client()

        response = client.get('/cwr/process/')
        self.assertEqual(response.status_code, 200)

    def test_post_no_data(self):
        client = self._app.test_client()

        response = client.post('/cwr/process/')
    ...
    def test_post_file_with_valid_data(self):
        ...
        data = {
            'filename': 'AB1234ABC_123.V21',
            'contents': _file_contents_cwr()
        }

        data = json.dumps(data)

        response = client.post('/cwr/process/', data=data,
                               headers={
